...
@interface ViewController ()
{
    ...
    __weak IBOutlet UILabel *txt2;
    __weak IBOutlet UILabel *txt3;
...
-(void)mzhBluetoothRecv_cb:(UInt16)characteristic_uuid :(NSData*)data
{
    if (Characteristic_Read==characteristic_uuid)
    {
        ...
        //显示数据
        memcpy(&cache_buf[cache_bufpos],[data bytes],[data length]);
        cache_bufpos+=[data length];
        if(cache_bufpos>400)
        {cache_bufpos=0;}
        //
        char *p=strchr(cache_buf, '}');
        if(p)
        {
            char srtt[256]={0};
            p++;
            long n=(long)p-(long)cache_buf;
            if(n>0 && n<=cache_bufpos)
            {
                memcpy(srtt,cache_buf,n);
                memmove(cache_buf,&cache_buf[n],n);
                cache_bufpos-=n;
                //
                cJSON *json;
                cJSON *jsonVal;
                json=cJSON_Parse(srtt);
                if (json)
                {
                    jsonVal=cJSON_GetObjectItem(json,"temperature");
                    txt1.text=[NSString stringWithFormat:@"%0.1f 度",jsonVal->valuedouble];
                    
                    jsonVal=cJSON_GetObjectItem(json,"trigger");
                    txt2.text=[NSString stringWithFormat:@"控制温度: %0.1f 度",jsonVal->valuedouble];
                    
                    cJSON_Delete(json);
                }
                //
                txt3.text=[self getDateTime];
            }
            else
            {
                cache_bufpos=0;
